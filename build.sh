...
PANDA_LLVM_BUILD="${PANDA_LLVM_BUILD:-Debug+Asserts}"
...
if [ "$PANDA_LLVM" != "" ]; then
  ...
  if llvm-config --version >/dev/null 2>/dev/null && [ $(llvm-config --version) == "3.8.0" ]; then
    echo "Found SYSTEM LLVM -- LLVM SUPPORT IS ENABLED"
    LLVM_BIT="--enable-llvm --with-llvm=$(llvm-config --prefix)"
  elif llvm-config-3.8 --version >/dev/null 2>/dev/null; then
    echo "Found SYSTEM LLVM -- LLVM SUPPORT IS ENABLED"
    LLVM_BIT="--enable-llvm --with-llvm=$(llvm-config-3.8 --prefix)"
  else
